...
- **Worker dies AFTER a job is queued/claimed:** the backend requeues stale profile, watchlist, and date-night claims after 5 minutes, then fails any job still active after 9 minutes so public polling reaches a clean terminal state before the browser's 10-minute deadline. Completed/failed jobs remain available for one hour before cleanup.
