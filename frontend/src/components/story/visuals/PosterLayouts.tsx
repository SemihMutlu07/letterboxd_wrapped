...
export function PosterCascade({ media, accent }: { media: StoryMedia[]; accent: string }) {
  ...
  return (
    <div className="relative h-full rotate-[7deg]">
      <div className="absolute inset-y-[-8%] left-[6%] right-[-8%] grid grid-cols-6 gap-3 md:left-[2%]">
...
export function HeroPoster({ media, accent }: { media: StoryMedia[]; accent: string }) {
  ...
  return (
    <div className="relative h-full">
      <div className="absolute left-[18%] top-1/2 aspect-[2/3] h-[78%] max-h-[82vh] -translate-y-1/2 md:left-[14%] lg:left-[12%]">
        <motion.div
      ...
      <div className="absolute bottom-[6%] left-[42%] flex gap-3 md:left-[48%]">
            ...
            transition={
              reduce
                ...
                : {
                    ...
                    y: { duration: 9 + index, repeat: Infinity, repeatType: 'reverse', ease: 'easeInOut' },
                  }
            }
...
export function PortraitStack({ media, accent }: { media: StoryMedia[]; accent: string }) {
  ...
  return (
    <div className="relative h-full">
      <div
        className="absolute left-[22%] top-1/2 aspect-[2/3] h-[82%] -translate-y-1/2 overflow-hidden rounded-[30px] border border-white/15 bg-black shadow-2xl md:left-[18%]"
        style={{ boxShadow: `0 0 90px ${accent}55` }}
      >
        <StoryImage item={first} priority />
      </div>
      <div className="absolute bottom-8 left-[48%] grid grid-cols-3 gap-3 md:left-[52%]">
    ...
    <div className="relative h-full">
      <motion.div
        ...
        transition={
          reduce
            ? { duration: 0 }
            : { y: { duration: 16, repeat: Infinity, ease: 'easeInOut' }, opacity: { duration: 0.5 } }
        }
        className="absolute left-[16%] top-[12%] z-20 aspect-[2/3] h-[58%] overflow-hidden rounded-[26px] border border-white/15 bg-black shadow-2xl"
      ...
      {secondary && (
        <motion.div
          ...
          transition={
            reduce
              ? { duration: 0 }
              : { y: { duration: 13, repeat: Infinity, ease: 'easeInOut' }, opacity: { duration: 0.55, delay: 0.12 } }
          }
          className="absolute bottom-[10%] left-[38%] z-30 aspect-[2/3] h-[42%] overflow-hidden rounded-[22px] border border-white/12 bg-black shadow-xl"
        >
          <StoryImage item={secondary} priority />
        </motion.div>
      )}
      <div className="absolute inset-y-[8%] left-[58%] right-[-4%] grid grid-cols-2 content-center gap-3">
        {stream.map((item, index) => (
          <motion.div
            ...
            transition={
              reduce
                ? undefined
                : { duration: 10 + index, repeat: Infinity, repeatType: 'reverse', ease: 'easeInOut' }
            }
