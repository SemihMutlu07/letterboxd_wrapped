...
function Marquee({ data }: { data: ShareCardData }) {
  ...
  return <div className="text-sm font-black uppercase leading-6" style={utilityFont}>{titles.map((title, i) => `${String(i + 1).padStart(2, '0')} ${title}`).join('  ·  ')}</div>;
}
...
const AdmitOneShareCard = React.forwardRef<HTMLDivElement, Props>(function AdmitOneShareCard(
  ...
  if (orientation === 'vertical') {
    return (
      <div ref={ref} data-export-root="true" className={`h-[1200px] w-[675px] overflow-hidden bg-[#c9963d] p-7 text-black ${className}`}>
        <div className="relative h-full border-[5px] border-[#153ea8] p-7">
          <div className="flex justify-between border-b-2 border-black pb-4 text-xs uppercase" style={utilityFont}><span>Movies Wrapped · 2026</span><Username username={data.username} /></div>
          ...
          {outlier && <div className="mt-7 border-2 border-[#153ea8] p-4 text-sm uppercase" style={utilityFont}>Row {Math.abs(Math.round(outlier.delta * 10))} · Seat {outlier.userRating}<br /><b className="text-xl">{outlier.title}</b></div>}
  ...
  return (
    <div ref={ref} data-export-root="true" className={`grid h-[675px] w-[1200px] grid-cols-[1fr_335px] overflow-hidden bg-[#c9963d] text-black ${className}`}>
      <main className="border-[6px] border-[#153ea8] p-9">
        <div className="flex justify-between border-b-2 border-black pb-4 text-xs uppercase" style={utilityFont}><span>Movies Wrapped · Valid 2026</span><Username username={data.username} /></div>
        ...
        <div className="mt-8 grid grid-cols-[1fr_250px] gap-10">
          ...
          {outlier && <div className="border-2 border-[#153ea8] p-4 text-sm uppercase" style={utilityFont}>Row {Math.abs(Math.round(outlier.delta * 10))} · Seat {outlier.userRating}<br /><b className="text-xl">{outlier.title}</b><br />Crowd {outlier.avgRating}</div>}
        </div>
      </main>
