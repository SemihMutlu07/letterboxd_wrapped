        ...
        <div className="mx-10 flex h-full min-w-0 flex-col py-[146px]">
          <header className="flex min-w-0 items-start justify-between gap-6 border-b-4 border-white pb-4">
            <div className="min-w-0">
              ...
              <h1 className="mt-1 text-[38px] font-black uppercase leading-none">Your Wrapped</h1>
            </div>
          ...
          <section className="mt-6 grid min-w-0 grid-cols-[minmax(0,1fr)_175px] items-end gap-6">
            <div className="min-w-0">
              <p className="text-[12px] font-black uppercase tracking-[0.18em] text-[#aab8b2]">Films watched</p>
              <strong className="block text-[112px] font-black leading-[0.8] tracking-[-0.055em] tabular-nums text-[#00e054]">
                {data.watchedFilms}
              </strong>
            </div>
            <p className="border-l border-white/30 pl-5 text-[18px] font-bold leading-snug">
              {data.writtenReviews} reviews<br />{data.spentDays} days<br />{data.spentHours} hours
            </p>
          </section>
          ...
          <section className="mt-5 grid min-h-0 min-w-0 flex-1 grid-cols-2 gap-3">
            <section className="flex min-w-0 flex-col items-center border border-[#00e054]/45 bg-[#15201c] p-4 text-center">
              ...
              <div className="mt-4 min-w-0 [overflow-wrap:anywhere]">
                <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#00e054]">On-screen crush</p>
                <p className="mt-1 text-[20px] font-black leading-tight">{data.onScreenCrush.name}</p>
                <p className="mt-1 text-[13px] text-[#aab8b2]">{data.onScreenCrush.count} films together</p>
              </div>
            </section>
            <section className="flex min-w-0 flex-col items-center border border-[#00e054]/45 bg-[#15201c] p-4 text-center">
              <PortraitFrame
              ...
              />
              <div className="mt-4 min-w-0 [overflow-wrap:anywhere]">
                <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#00e054]">Favorite director</p>
                <p className="mt-1 text-[20px] font-black leading-tight">{data.favoriteDirector.name}</p>
                <p className="mt-1 text-[13px] text-[#aab8b2]">{data.favoriteDirector.count} films directed</p>
              </div>
            </section>
          </section>
