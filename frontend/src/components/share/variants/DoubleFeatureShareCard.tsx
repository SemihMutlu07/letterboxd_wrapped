  ...
  if (vertical) {
    return (
      <div ref={ref} data-export-root="true" className={`relative h-[1200px] w-[675px] overflow-hidden bg-[#10100f] p-8 text-[#f1e7d0] ${className}`}>
        <header className="flex items-start justify-between gap-6 border-b-4 border-[#ef4b2f] pb-5">
          <div className="min-w-0 max-w-[430px]"><div className="text-sm uppercase tracking-[0.3em]" style={utilityFont}>Movies Wrapped · Double Feature</div><Username username={data.username} /></div>
          <div className="shrink-0 text-right text-xs uppercase" style={utilityFont}>{data.peakDecade}<br />Program 01</div>
        </header>
        ...
        <PosterSlots films={data.topFilms} className="mt-6 gap-2" slotClassName="h-[189px]" />
      </div>
    );
  }
  return (
    <div ref={ref} data-export-root="true" className={`grid h-[675px] w-[1200px] grid-cols-[430px_1fr] overflow-hidden bg-[#10100f] text-[#f1e7d0] ${className}`}>
      ...
      <div className="flex min-w-0 flex-col overflow-hidden px-5 py-6">
        <div className="flex justify-between gap-6 border-b-4 border-[#ef4b2f] pb-4 text-xs uppercase tracking-[0.25em]" style={utilityFont}><span className="shrink-0">Double Feature</span><div className="min-w-0 max-w-[430px] text-right"><Username username={data.username} /></div></div>
        <div className="mt-5 text-[64px] font-black uppercase leading-[0.85]" style={displayFont}>Your year<br />in pictures</div>
        <div className="-mt-2 text-[150px] font-black leading-none text-[#ef4b2f]" style={displayFont}>{data.watchedFilms}</div>
        ...
        <PosterSlots films={data.topFilms} className="mt-4 gap-2" slotClassName="h-[120px]" />
      </div>
    </div>
  );
});
