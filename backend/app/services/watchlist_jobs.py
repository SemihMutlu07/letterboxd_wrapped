...
def _is_current_processing_task(task_id: str, task) -> bool:
    return task_manager.get_task_state(task_id) is task and task.status == "running" and task.stage == "processing"
    ...
    try:
        if task.job_type == "watchlist_compare":
            ...
            if not recommendations:
                raise WatchlistProcessingError("No strong mutual recommendation was found yet.", "no_recommendations")
        ...
        task_manager.set_task_failed(task_id, str(exc), {
            "error_type": type(exc).__name__, "error_stage": "processing", "error_code": error_code,
        })
        await log_worker_event("watchlist_finalization_failed", {
            "source": "backend", "severity": "error", "task_id": task_id, "job_type": task.job_type,
            "message": str(exc), "error_type": type(exc).__name__, "error_stage": "processing", "error_code": error_code,
        })
