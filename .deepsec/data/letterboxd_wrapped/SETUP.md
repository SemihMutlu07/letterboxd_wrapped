# Agent setup for `letterboxd_wrapped`

This is a deepsec scanning workspace. Project `letterboxd_wrapped` was just registered
(target: `..`). Setup is incomplete — `data/letterboxd_wrapped/INFO.md`
...
2. **Fill in `data/letterboxd_wrapped/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id letterboxd_wrapped
pnpm deepsec process --project-id letterboxd_wrapped
```
