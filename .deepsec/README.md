...
Currently configured project: `letterboxd_wrapped` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/letterboxd_wrapped/SETUP.md` to fill in
   `data/letterboxd_wrapped/INFO.md`.
...
`--project-id letterboxd_wrapped` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/letterboxd_wrapped/`.
